...
function surf(seed, baseElev = 3050) {
    return {
...
function createMonitoringPoints() {
    ...
    PATTERN_SITES.forEach((p, pi) => {
        ...
        for (let i = 0; i < 3; i++) {
            const r = ring(p.lat, p.lon, 0.45, i, 3);
            s++;
        ...
        for (let i = 0; i < 2; i++) {
            const r = ring(p.lat, p.lon, 0.7, i, 2);
            s++;
        ...
        for (let i = 0; i < 2; i++) {
            const r = ring(p.lat, p.lon, 0.35, i + 1, 4);
            s++;
        ...
        for (let i = 0; i < 4; i++) {
            const r = ring(p.inj.lat, p.inj.lon, 0.3, i, 4);
            s++;
...
function createSurfaceStructures() {
    ...
    PATTERN_SITES.forEach((p) => {
        s++;
        const tb = ring(p.lat, p.lon, 0.25, 0, 1);
        ...
        s++;
        const sk = ring(p.lat, p.lon, 0.25, 2, 4);
        ...
        s++;
        const sep = ring(p.lat, p.lon, 0.18, 1, 4);
...
// ------------------------------------------------------------------
function createGatheringLines() {
    const cpf = [BASE_LON + dLon(0.5), BASE_LAT];
    return PATTERN_SITES.map((p) => ({
        id: `GL-${p.id}`, name: `${p.name} Gathering Flowline`, product: 'mixed',
        fromId: p.id, toId: 'FAC-CPF', from: [p.lon, p.lat], to: cpf, diameterIn: 6,
    }));
}
// ------------------------------------------------------------------
// Lease boundary — polygon enclosing the field
// ------------------------------------------------------------------
function createLeaseBoundary() {
