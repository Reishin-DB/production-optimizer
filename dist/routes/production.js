...
const decline_1 = require("../physics/decline");
const darcy_1 = require("../physics/darcy");
const router = (0, express_1.Router)();
const provider = new provider_1.InMemoryTwinDataProvider();
const DATABRICKS_AVAILABLE = (0, sql_1.isDatabricksAvailable)();
let databricksWorking = false; // tracks if DB queries actually succeed
if (DATABRICKS_AVAILABLE)
    console.log('Databricks detected — will attempt Unity Catalog tables');
else
    console.log('No Databricks credentials — using in-memory mock data');
/** Try Databricks, fall back to mock on any failure */
async function tryDatabricks(dbFn, mockFn) {
    if (DATABRICKS_AVAILABLE) {
        try {
 ...
 */
router.get('/decline-curves', async (_req, res) => {
    const now = Date.now();
    ...
    try {
        ...
        const results = producers.map((well, idx) => {
            ...
            const history = (0, decline_1.generateProductionHistory)(well.id, well.name, well.oilRate, monthsOn, seed);
            // Fit Arps model to the generated history
            const historyForFit = history.map(h => ({ month: h.month, rate: h.actual }));
            const fit = (0, decline_1.fitDeclineCurve)(historyForFit);
            ...
            for (let m = 1; m <= 36; m++) {
                const t = lastMonth + m;
                const predicted = (0, decline_1.arpsRate)(fit.params, t);
            ...
            // EUR calculation
            const eur = (0, decline_1.calculateEUR)(fit.params, 5); // 5 bbl/d economic limit
...
router.get('/well-analytics', async (_req, res) => {
    const now = Date.now();
    ...
    try {
        ...
        if (DATABRICKS_AVAILABLE) {
            try {
                ...
                const wells = await (0, sql_1.executeQuery)(`SELECT * FROM ${(0, sql_1.table)('bronze_wells')} WHERE well_type = 'producer'`);
                const historyRows = await (0, sql_1.executeQuery)(`SELECT * FROM ${(0, sql_1.table)('silver_production_history')} ORDER BY well_id, month`);
                ...
                results = wells.map((well) => {
                    const hist = histByWell[well.well_id] || [];
                    const monthsOn = hist.length > 0 ? hist.length - 1 : 24;
                    // Fit Arps to actual Databricks data
                    const histForFit = hist.map((h) => ({ month: h.month, rate: h.oil_rate }));
                    const fit = (0, decline_1.fitDeclineCurve)(histForFit);
                    const eur = (0, decline_1.calculateEUR)(fit.params, 5);
                    const lastActual = hist.length > 0 ? hist[hist.length - 1].oil_rate : well.oil_rate;
                    const lastPredicted = (0, decline_1.arpsRate)(fit.params, monthsOn);
                    ...
                    if (well.water_cut > 0.7)
                        health -= 20;
                    else if (well.water_cut > 0.5)
                        health -= 10;
                    if (well.co2_concentration > 15)
                        health -= 15;
                    else if (well.co2_concentration > 8)
                        health -= 5;
                    health = Math.max(0, Math.min(100, Math.round(health)));
                    // Map DB rows to analytics points
                    const history = hist.map((h) => ({
                        month: h.month, date: h.production_date,
                        oilRate: h.oil_rate, oilPredicted: (0, decline_1.arpsRate)(fit.params, h.month),
                    ...
                    for (let m = 1; m <= 24; m++) {
                        const ft = monthsOn + m;
                        const oilPredicted = (0, decline_1.arpsRate)(fit.params, ft);
        ...
        if (!usedDatabricks) {
            ...
            const state = await provider.loadState();
            const producers = state.wells.filter((w) => w.type === 'producer');
            results = producers.map((well, idx) => {
                const monthsOn = 24 + (idx * 3);
                const seed = well.id.charCodeAt(2) * 1000 + well.id.charCodeAt(3);
                return (0, decline_1.generateWellAnalytics)(well.id, well.name, well, monthsOn, seed);
            });
        }
...
router.post('/what-if', async (req, res) => {
    try {
        ...
        const targetPattern = req.body.patternId;
        const state = await provider.loadState();
        // Get patterns and their wells
        const patterns = targetPattern
            ? state.patterns.filter((p) => p.id === targetPattern)
            : state.patterns;
        const results = [];
        let totalDailyImpact = 0;
        for (const pattern of patterns) {
            const reservoir = (0, darcy_1.getDefaultReservoirParams)(pattern.id);
            const producers = state.wells.filter((w) => w.type === 'producer' && w.patternId === pattern.id);
            const injectors = state.wells.filter((w) => (w.type === 'injector' || w.type === 'WAG') && w.patternId === pattern.id);
            const totalInjRate = injectors.reduce((s, w) => s + (w.co2InjRate || 0), 0);
            // Calculate economics for each producer
            const fieldOilRate = state.wells
                .filter((w) => w.type === 'producer')
                .reduce((s, w) => s + w.oilRate, 0);
            for (const well of producers) {
                ...
                const result = (0, darcy_1.evaluateWhatIf)(well.id, well.name, well.oilRate, pattern.currentPressure, totalInjRate / Math.max(1, producers.length), // per-well allocation
                netback, reservoir, scenario);
                results.push(result);
        ...
        res.json({
            ...
            summary: {
                ...
                avgOilRateChange: Math.round(results.reduce((s, r) => s + r.oilRateChange, 0) / results.length * 10) / 10,
 ...
 */
router.get('/reservoir-status', async (_req, res) => {
    try {
        const state = await provider.loadState();
        const patternStatus = state.patterns.map((pattern) => {
            const reservoir = (0, darcy_1.getDefaultReservoirParams)(pattern.id);
            const injectivity = (0, darcy_1.calculateInjectivity)(reservoir, pattern.currentPressure, 2);
            const producers = state.wells.filter((w) => w.type === 'producer' && w.patternId === pattern.id);
            ...
            const mb = (0, darcy_1.calculateMaterialBalance)(reservoir, pattern.currentPressure, estCumulativeOil, estCumulativeCO2);
            return {
    ...
    try {
        ...
        if (DATABRICKS_AVAILABLE) {
            try {
                ...
                const wells = await (0, sql_1.executeQuery)(`SELECT * FROM ${(0, sql_1.table)('bronze_wells')}`);
                const patterns = await (0, sql_1.executeQuery)(`SELECT * FROM ${(0, sql_1.table)('bronze_patterns')}`);
                state = {
                    wells: wells.map((w) => ({
                ...
                for (const p of state.patterns) {
                    p.producerIds = state.wells.filter((w) => w.patternId === p.id && w.type === 'producer').map((w) => w.id);
                    p.injectorIds = state.wells.filter((w) => w.patternId === p.id && (w.type === 'injector' || w.type === 'WAG')).map((w) => w.id);
                }
            }
            catch (dbErr) {
        ...
        for (const pattern of state.patterns) {
            ...
            const totalOilRate = producers.reduce((s, w) => s + w.oilRate, 0);
            const totalInjRate = injectors.reduce((s, w) => s + (w.co2InjRate || 0), 0);
            // Rule 1: Pressure below target — increase injection
            if (pressureDeficit > 100) {
                const inj = (0, darcy_1.calculateInjectivity)(reservoir, pattern.currentPressure);
                ...
                recommendations.push({
                    ...
                    affectedEntities: [...injectors.map((w) => w.id), ...producers.map((w) => w.id)],
                    estimatedImpact: {
            ...
            if (pattern.currentPhase === 'CO2_injection' && pattern.cycleNumber > 3) {
                const avgCO2Concentration = producers.reduce((s, w) => s + (w.co2Concentration || 0), 0) / producers.length;
                if (avgCO2Concentration > 8) {
                    recommendations.push({
                        ...
                        affectedEntities: injectors.map((w) => w.id),
                        estimatedImpact: {
        ...
        const totalAnnualImpact = recommendations.reduce((s, r) => s + r.estimatedImpact.annualRevenue, 0);
        res.json({
            recommendations,
            summary: {
