...
export interface MonitoringPoint extends GeoPoint, SurfaceData {
  ...
  patternId?: string;       // pattern this station monitors, when applicable
}
...
export interface SurfaceStructure extends GeoPoint, SurfaceData {
  ...
  throughput: number;       // context value (bbl/d or mcf/d)
  unit: string;
  padId?: string;
}
...
export interface TwinState {
  ...
  leaseBoundary: [number, number][];   // ring of [lon, lat]
  flares: FlarePoint[];
  fleet: FleetAsset[];
  alerts: Alert[];
  shiftLog: ShiftLog;
